...
'''
    TO-DO: 
        [ ] use yield to show files as script works
        [ ] add exclusions
        [ ] ADD ABILITY/FUNCTION TO SEARCH AND RETURN FILES WITH SAME NAME
        ...
        [ ] add ability to find files based on file name
        [ ] expand help function
        [ ] args and switches work
...
if __name__ == "__main__":
    ...
    Searches contents of files for key words. 
    Capable of batch searching individual directories and 
    ...
    if (len(args) > 0):
        ...
        for i in range(len(args)):
            ...
            if (args[i] in variable_dict.keys()):
                ...
                if not (i == (len(args) - 1)):
                    print('ignoring {}: not a valid switch'.format(args[i]))
